...
def has_card(to_check, start, end):
    if start > end:
        return 0

    mid = int((start + end) >> 1)

    # print("start: {0}, end: {1}, mid: {2}, hold[{2}]: {3}, to_check: {4}".format(start, end, mid, hold[mid], to_check))

    if hold[mid] == to_check:
        return 1

    if hold[mid] > to_check:
        return has_card(to_check, start, mid - 1)
    else:
        return has_card(to_check, mid + 1, end)


for card in check_list:
    answer.append(has_card(card, 0, N - 1))
