...
# 입력: 첫째 줄에 포도주 잔의 개수 n이 주어진다. (1≤n≤10,000)
...
for i in range(1, n + 1):
    amount[i] = int(sys.stdin.readline().rstrip())

dp = [0] * (10000 + 1)      # dp[i]: i번째 잔까지 마셨을 때 최대한 마셨을 경우의 포도주의 양
dp[1] = amount[1]
dp[2] = dp[1] + amount[2]

# 1. i번째 잔을 마시지 않았을 때 (0번 연속)
# dp[i] = dp[i - 1]
# 2. i-1번째 잔을 마시지 않고 i번쨰 잔을 마셨을 때(1번 연속)
# dp[i] = dp[i - 2] + amount[i]
# 3. i-2번째 잔을 마시지 않고 i-1번째 잔과 i번째 잔을 마셨을 때(2번 연속)
