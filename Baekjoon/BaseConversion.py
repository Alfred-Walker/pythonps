...
def convert_base(num, base_original, base_target):
    ret = []
    num_10 = 0
    length = len(num)
    i = length - 1
    while True:
        num_10 += num[i] * (base_original ** (length - i - 1))
        i -= 1
    ...
    div = num_10
    while True:
        div, mod = divmod(div, base_target)
        ret.append(mod)
        if div == 0:
            return ret
...
num_A = list(map(int, sys.stdin.readline().rstrip().split()))
print(*reversed(convert_base(num_A, A, B)))
