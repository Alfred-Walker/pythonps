...
for i in range(len(S)):
    ...
    def set_lotto_comb(start, end, current, lis):
        global result
        # print("start: {0}   end: {1}    current: {2}    lis: {3}".format(start, end, current, lis))
        if len(current) == 6:
        ...
        set_lotto_comb(start + 1, end, current[:], lis)
        current.append(lis[start])
        set_lotto_comb(start + 1, end, current[:], lis)


    set_lotto_comb(0, K[i], [], S[i])
    result.sort()
