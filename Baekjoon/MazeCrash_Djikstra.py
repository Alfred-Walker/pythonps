...
def djikstra():
    priority_queue = []
    heappush(priority_queue, (0, 0, 0))     # crashed, y, x
    crashed[0][0] = 0

    while priority_queue:
        c, y, x = heappop(priority_queue)
        for i in range(4):
            next_y = y + dy[i]
            next_x = x + dx[i]

            if 0 <= next_y < N and 0 <= next_x < M:
                crash_cnt = c + int(maze[next_y][next_x])

                if crashed[next_y][next_x] > crash_cnt:
                    crashed[next_y][next_x] = crash_cnt
                    heappush(priority_queue, (crash_cnt, next_y, next_x))

    return crashed[N - 1][M - 1]
